...
class Tester(object):
    ...
    def test_one_step(self, data):
        if self.other_model == None:      
            ...
            # Ensemble Inference
            score1 = self.model.predict({
                'batch_h': self.to_var(data['batch_h'], self.use_gpu),
                'batch_t': self.to_var(data['batch_t'], self.use_gpu),
                'batch_r': self.to_var(data['batch_r'], self.use_gpu),
                'mode': data['mode']
            })
            score2 = self.other_model.predict({
                'batch_h': self.to_var(data['batch_h'], self.use_gpu),
                'batch_t': self.to_var(data['batch_t'], self.use_gpu),
                'batch_r': self.to_var(data['batch_r'], self.use_gpu),
                'mode': data['mode']
            })
            return self.mu * score1 + (1-self.mu) * score2
            """
            if self.norm == True:
                score = F.softmax(torch.tensor(score1), dim=0) + F.softmax(torch.tensor(score2), dim=0)
                return  score.numpy()
            else:
